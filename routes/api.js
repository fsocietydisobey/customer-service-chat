...
router.put('/agents/status', authMiddleware, async (req, res) => {
    try {
        ...
        req.log.error('Error updating agent status via API:', error);
...
router.get('/chat_sessions', authMiddleware, async (req, res) => {
    try {
        ...
        req.log.error('Error fetching chat sessions via API:', error);
        res.status(500).send('Server Error');
    }
});
...
router.get('/agents', authMiddleware, async (req, res) => {
    try {
        ...
        req.log.error('Error fetching agents via API:', error);
        res.status(500).send('Server Error');
    }
});
...
router.get('/agents/all', authMiddleware, async (req, res) => {
    try {
        ...
        req.log.error('Error fetching all agents via API:', error);
        res.status(500).send('Server Error');
    }
});
...
router.get('/chat_sessions/:sessionId/messages', authMiddleware, async (req, res) => {
    try {
        ...
        req.log.error('Error fetching session messages via API:', error);
        res.status(500).send('Server Error');
    }
});
